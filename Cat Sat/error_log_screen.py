...
class ErrorLogScreen(ctk.CTkFrame):
    ...
    def _save_fix(self, attempt_id, var, status, row, event=None):
        """
        Reject the non-instruction. "Read carefully" is the correct diagnosis
        and a useless instruction: it restates the error instead of correcting
        it, so the same misses keep recurring. A fix has to be something you can
        actually execute on the next question.
        """
        text = var.get().strip()
        if not text:

...
def estimate_section_score(correct: int, total: int, final_tier: str) -> int:
    """
    Routing-aware estimated section score, rounded to the nearest 10.

    College Board does not publish its conversion tables and they differ per
    form, so this is deliberately labelled an estimate everywhere it appears.
    What it does model correctly is the important structural fact: taking the
    lower-route Module 2 puts a ceiling on your section score.
    """
    if total <= 0:
        return 0
    model = SCORE_MODEL.get(final_tier, SCORE_MODEL[TIER_BASELINE])
    pct = max(0.0, min(1.0, correct / total))
    # Slight concavity: the last few questions are worth more than the first few.
    shaped = pct ** 0.9
    raw = model["base"] + model["span"] * shaped
    clamped = max(model["floor"], min(model["ceiling"], raw))

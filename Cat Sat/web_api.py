...
def summary_json(summary) -> dict:
    ...
    for outcome in summary.sections:
        sections.append({
            ...
            "scoreReliable": outcome.score_is_reliable,
            "modules": [{
...
class Api:
    ...
    def bootstrap(self) -> dict:
        ...
        return {
            # bool(), not the truthy thing question_bank_is_usable happens to
            # return. The browser tests this with === false to tell "no bank"
    ...
    def plan(self, day_iso: str | None = None) -> dict:
        ...
        return {
            ...
            # The plan's real date span, so the UI never has to hardcode one.
            # A hardcoded window is somebody else's exam schedule shipped to
            # every downloader.
            "planSpan": _plan_span(),
...
def _plan_span() -> str | None:
    """e.g. 'Mar 01 to Jun 06, 2027' for THIS user's plan; None if they have none."""
    weeks = study_plan.active_weeks()
    if not weeks:
        return None
    start, end = weeks[0]["start"], weeks[-1]["end"]
    return f"{start.strftime('%b %d').replace(' 0', ' ')} to {end.strftime('%b %d, %Y').replace(' 0', ' ')}"

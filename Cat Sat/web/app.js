...
function barRow(label, correct, total) {
  ...
  return el('div.barrow',
    el('div.top', el('span', label), el('span', { style: { color: accColor(p) } }, `${pct(p)}  (${correct}/${total})`)),
    el('div.bar', el('i', { style: { width: `${p}%`, background: accColor(p) } })));
}
...
function renderNav() {
  $('#nav').replaceChildren(...ROUTES.map(([key, label]) =>
  ...
  $('#topright').replaceChildren(...right);
}

async function refreshBoot() {
  State.boot = await api('bootstrap', { quiet: true });
  renderNav();
}
...
function go(route, arg) {
  ...
  let settled = false;
  setTimeout(() => { if (!settled) screenEl().replaceChildren(el('div.spin')); }, 180);
  Promise.resolve(view(arg)).finally(() => { settled = true; });
}
...
function renderQuiz(payload) {
  ...
  function paintQuestion() {
    ...
    if (q.image) { img.src = q.image; img.style.display = ''; imgWrap.replaceChildren(img); }
    else imgWrap.replaceChildren(el('div.qmissing', '[ Question image missing ]'));
    applyZoom();

    if (q.openEnded) {
      answers.replaceChildren(el('div.gridin', gridIn));
      gridIn.value = Q.answers[Q.index] || '';
    } else {
      answers.replaceChildren(choicesEl);
      paintChoices();
    }
    paintState();
    prefetch();
  ...
  function paintChoices() {
    ...
    choicesEl.replaceChildren(...['A', 'B', 'C', 'D'].map((letter) => {
  ...
  async function submit(auto) {
    ...
    bankTime();
    screenEl().replaceChildren(el('div.spin'));
    const body = {
      answers: Q.answers,
      times: Q.times,
      flagged: [...Q.flagged],
  ...
  function recordRow(r) {
    return el('div.item',
      el('div.row',
        ...
        !r.isCorrect ? el('span', { style: { color: 'var(--green)', fontWeight: 700 } }, `Correct: ${r.correctAnswer}`) : null,
    ...
    nodes.push(el('div.card',
      ...
      el('div.grid.c3', { style: { marginTop: '8px' } },
        summary.sections.filter((s) => s.total).map((s) =>
          tile(s.section, s.estimatedScore, accColor(s.accuracy), `${s.correct}/${s.total} · ${s.scoreBand}`)),
...
function showExplanation(r) {
  modal(`📖 Explanation — ${r.domain} · ${r.difficulty}`, [
    el('div.row', el('span', { style: { color: r.isCorrect ? 'var(--green)' : 'var(--red)', fontWeight: 700 } }, `Your answer: ${r.selected || '— skipped —'}`),
      el('span', { style: { color: 'var(--green)', fontWeight: 700 } }, `Correct answer: ${r.correctAnswer}`)),
    ...
    r.rationale ? el('img', { src: r.rationale, style: { background: '#fff', borderRadius: '8px' } })
  ...
  mount(
    ...
    el('div.grid.c2',
      el('div.card', el('h2', 'Domains'), el('p.faint', 'weakest first'),
        el('div', { style: { marginTop: '8px' } }, d.domains.map((b) => barRow(b.bucket, b.correct, b.total)))),
...
(async function boot() {
  await refreshBoot();
  if (!State.boot.bankOk) {
    renderNav();
    return mount(empty('⚠️', 'No question bank yet', State.boot.bankMessage,
      'Retry', () => location.reload()),
      el('div.card', el('h2', 'How to fix it'),
        el('p.sub', 'Put the College Board PDFs (downloaded with answers and rationales) in the pdfs folder, then run:'),
        el('pre', { style: { background: 'var(--surface3)', padding: '12px', borderRadius: '9px', marginTop: '8px' } },
